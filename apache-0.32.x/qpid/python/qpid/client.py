    ...
    def connection_start(self, ch, msg):
        ...
        if self.client.mechanism is None:
            if self.client.response is not None:
                ...
                self.client.sasl = get_sasl_mechanism(supportedMechs, self.client.username, self.client.password,
                                                      sasl_options=self.client.sasl_options)

                if self.client.sasl == None:
                    raise SaslException(
                        "sasl negotiation failed: no mechanism agreed.  Server supports: %s " % supportedMechs)
